...
def _block_name_base(stage, block, _name_counter=Counter()):
    ...
    """
    _name_counter.update([(stage, block)])
    i = _name_counter[(stage, block)]
    if block < 27:
        block = '%c' % (block + 97)  # 97 is the ascii number for lowercase 'a'
    conv_name_base = 'res' + str(i) + str(stage) + block + '_branch'
    bn_name_base = 'bn' + str(i) + str(stage) + block + '_branch'
...
def ResNet(input_shape=None, classes=10, block='bottleneck', residual_unit='v2', repetitions=None,
           initial_filters=64, activation='softmax', include_top=True, input_layer=None, input_tensor=None, dropout=None,
